...
(function (root, factory) {
  ...
  select.strings = {
    ...
    ru: {
      ...
      placeholderMulti: function (args) {
        var strings = ["варианта", "вариантов", "вариантов"],
          cases = [2, 0, 1, 1, 1, 2],
          num = args[":limit"];
        var string =
          strings[
  ...
  select.prototype = {
    ...
    init: function () {
      var self = this,
        ...
        con = this.con,
        regexp =
          /^[0-9.]+(?:cm|mm|in|px|pt|pc|em|ex|ch|rem|vw|vh|vmin|vmax|\%)$/i;
      ...
      if (con.search) {
        ...
        this.search.children[0].addEventListener("input", function (event) {
          self.query.call(
            self,
            this.value.length > con.searchMinLength ? this.value : undefined
          );
        });
    ...
    bind: function () {
      ...
      d.addEventListener("keydown", function (event) {
        ...
        if (
          !space &&
          (!cHAS(self.select, "active") || [13, 27, 38, 40].indexOf(key) < 0)
        ) {
          return false;
        }
        ...
        if (
          ...
          do {
            ...
            if (
              opt &&
              (!cHAS(opt, "dropdown-option") || cHAS(opt, "disabled"))
            ) {
              continue;
            }
            break;
        ...
        if (!opt && key == 40) {
          ...
          tmp = self.dropdown.querySelectorAll(
      ...
      d.addEventListener("click", function (ev) {
        ...
        for (var l = targets.length, i = 0; i < l; i++) {
          if (
            targets[i] &&
            (targets[i].contains(ev.target) || targets[i] == ev.target)
          ) {
            return false;
          }
    ...
    updatePin: function (item) {
      var inner = this.dropdown.querySelector(".dropdown-inner ul"),
        option =
          "li[data-key='" + item.key + "'][data-group='" + item.group + "']";
  ...
  options.prototype = {
    ...
    get: function (key, grp) {
      ...
      if (typeof key == "object" && key.key && key.group) {
        ...
        if (key.tagName == "OPTION") {
          ...
          grp =
            key[g]("data-group") || key.parentElement[g]("data-group") || "#";
          key = key[g]("data-key");
        }
    ...
    set: function (opt, rebuild) {
      ...
      return rebuild
        ? this.self.callback(this.items[grp][key], "rebuild")
        : true;
    },
    ...
    add: function (
      ...
      if (
        ...
        this.selected.length >= this.self.con.multiLimit
      ) {
        selected = false;
      }
      disabled = !!disabled;
    ...
    handle: function (state, key, group, _force) {
      ...
      var dis =
        ...
        uns =
          !this.self.con.multiple && !this.self.con.deselect && _force !== true;
      if (state == "select") {
        if (dis || lmt || del || sgl) {
          return false;
        }
      ...
      if (items instanceof Array || items.length) {
        ...
        if (items.forEach) {
          ...
          for (var key in items) {
            ...
            this.handle
              .apply(this, [state, items[key], key in this.items ? key : null])
              .concat(args);
          }
        }
      }
    ...
    applyLinguisticRules: function (search, casesensitive) {
      var rules = this.self.con.linguisticRules,
        values = [];
